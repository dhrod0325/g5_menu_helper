			...
			for ( $i = 0; $row = sql_fetch_array( $result ); $i ++ ) {
				...
				<tr class="<?php echo $bg; ?> menu_list menu_group_<?php echo substr( $row['me_code'], 0, 2 ); ?>">
					<td class="td_category<?php echo $sub_menu_class; ?>">
						...
						<label for="me_name_<?php echo $i; ?>" class="sound_only"><?php echo $sub_menu_info; ?>메뉴<strong
								class="sound_only"> 필수</strong></label>
						<input type="text" name="me_name[]" value="<?php echo $row['me_name'] ?>"
					...
					<td class="td_mng">
						...
						<select name="me_target[]" id="me_target_<?php echo $i; ?>">
							...
							<option value="blank"<?php echo get_selected( $row['me_target'], 'blank', true ); ?>>사용함
							</option>
						</select>
					</td>
					<td class="td_num">
					...
					<td style="font-size:10px;width:250px;">
						<label for="me_out_image_<?= $i ?>" class="">아웃</label>
						<input type="file" name="me_out_image[]" id="me_out_image_<?= $i ?>" style="font-size:10px;"/>
						<?php if ( ! empty( $row['me_out_image'] ) ) { ?>
							<img src="<?= $row['me_out_image'] ?>" alt=""/>
						<?php } ?>

						<br/><br/>

						<label for="me_over_image_<?php echo $i; ?>" class="">오버</label>
						<input type="file" name="me_over_image[]" id="me_over_image_<?= $i ?>" style="font-size:10px;"/>
						<?php if ( ! empty( $row['me_over_image'] ) ) { ?>
							<img src="<?= $row['me_over_image'] ?>" alt=""/>
						<?php } ?>

						<input type="hidden" name="me_over_image_url[]" value="<?= $row['me_over_image'] ?>"/>
						<input type="hidden" name="me_out_image_url[]" value="<?= $row['me_out_image'] ?>"/>
					</td>
